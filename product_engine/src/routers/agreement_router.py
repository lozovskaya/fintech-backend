...
class AgreementCBV:
    ...
    def create_agreement(self, agreement: AgreementRequest) -> int:
        # Check if the product with the given code already exists
        product = crud_products.get_product_by_internal_code(self.db, agreement.product_code)
        if not product:
        ...
        client = crud_clients.get_client_by_all_data(self.db, agreement.client)
        if client is None:
            client_id = crud_clients.create_client(self.db, agreement.client)
            if not client_id:
        ...
        # Create agreement
        new_agreement = crud_agreements.create_agreement(self.db, AgreementModel(
